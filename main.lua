...
function MakeFakeInstFunc(Result)
	return function(...)
		local Arguments = {...}
		for I, V in pairs (Arguments) do
			if type(V) == "userdata" then
...
function InitializeHooksInCurrentEnvironment()
	...
	local NewEnvironment = setmetatable({}, {
		__index = function(self, key)
			local Result = CurrentEnvironment[key]
			if typeof(Result) == "Instance" then
				return MakeFakeInstance(Result)
			end
			if typeof(Result) == "function" then
				return MakeFakeInstFunc(Result)
			end
			if typeof(Result) == "table" then
				return LoopAllTBLs(Result, function(func)
					if typeof(func) == "function" then
						return MakeFakeInstFunc(func)
					end
				end)
			end
			if Result == nil then
